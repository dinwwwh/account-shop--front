<template>
  <GroupSeparated v-if="recharge_phonecard_thesieure_enable">
    <div class="space-y-4">
      <div>
        <HeadingBase4> Nạp thẻ tự động (Thesieure.com) </HeadingBase4>
        <DescriptionBase>
          Thẻ được phê duyệt một cách tự động (2p/1thẻ).
        </DescriptionBase>
      </div>
      ...
      <div class="space-y-2">
        <SelectBase
          ...
          :options="recharge_phonecard_tsr_telcos"
          display-key="name"
          value-key="key"
...
export default {
  data() {
    return {
      ...
      recharge_phonecard_tsr_telcos: [],
    };
  },
  async fetch() {
    const { data: telcos } = await this.$axios.$get('thesieure/telcos');
    this.recharge_phonecard_tsr_telcos = telcos;
  },
  computed: {
    faceValueOptionsForManualCard() {
      const selectedTelco = this.recharge_phonecard_tsr_telcos.find(
    ...
    recharge_phonecard_thesieure_enable() {
      return this.$dynamicConfig('recharge-phonecard.port_thesieure_enable');
    },
  },
  validations() {
    return {
